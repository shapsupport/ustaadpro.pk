...
const faqs = [
  {
    ...
    answer:
      "Our online store is currently in development and will launch soon with secure payments, nationwide delivery across Pakistan, and competitive pricing. Subscribe to our newsletter to be the first to know!",
  },
  {
    question: "What payment methods will be supported?",
    answer:
      "We will support Cash on Delivery (COD), Debit/Credit Cards, JazzCash, EasyPaisa, and bank transfers to make shopping convenient for everyone across Pakistan.",
  },
  {
    question: "Will you deliver nationwide?",
    answer:
      "Yes! We're building a nationwide delivery network to ensure products reach every corner of Pakistan. Major cities will have express delivery options available.",
  },
  {
    question: "Can I return products?",
    answer:
      "We will have a hassle-free return and refund policy. Details will be shared closer to the store launch date.",
  },
  {
    question: "Are all products genuine and branded?",
    answer:
      "Absolutely. We only source products from authorized distributors and verified brands. Every product comes with manufacturer warranty.",
  },
];

async function getShopData(): Promise<{
  products: ApiProduct[];
  categories: ApiShopResponse["categories"];
}> {
  try {
    const res = await fetch(`${API_BASE_URL}/api/shop/products?limit=20`, {
...
export default async function StorePage() {
  ...
  const validProducts: ApiProduct[] = allProducts.filter(
    (p: ApiProduct) => p && p.id,
  );
            ...
            <MotionWrapper>
              <div className="max-w-3xl">
                ...
                <p className="mt-4 text-lg leading-8 text-gray-300">
                  Browse curated tools, paints, hardware, and maintenance
                  essentials from our live product catalog with fast support and
                  reliable delivery.
                </p>
                <div className="mt-8 flex flex-wrap items-center gap-4">
                  <Link
                    href="#catalog"
                    className="inline-flex items-center gap-2 rounded-full bg-lime-500 px-5 py-3 text-sm font-semibold text-gray-950 transition hover:bg-lime-400"
                  >
                  ...
                  <div className="mt-6 grid gap-3 sm:grid-cols-2">
                    <div className="rounded-2xl border border-white/10 bg-white/10 p-4">
                      ...
                      <p className="mt-2 text-sm text-gray-300">
                        Nationwide shipment for your essentials.
                      </p>
                    </div>
                    <div className="rounded-2xl border border-white/10 bg-white/10 p-4">
                      <div className="flex items-center gap-2 text-lime-300">
                      ...
                      <p className="mt-2 text-sm text-gray-300">
                        Simple returns if the product does not meet
                        expectations.
                      </p>
                    </div>
                  </div>
            ...
            <div className="mt-8 grid gap-4 sm:grid-cols-2 lg:grid-cols-3">
              {apiCategories.map((cat, index) => {
                ...
                return (
                  <Link
                    key={`${cat.name}-${index}`}
                    href={`/store/category/${cat.name.toLowerCase().replace(/\s+/g, "-")}`}
                  >
        ...
        <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
          <div className="flex flex-col gap-4 sm:flex-row sm:items-end sm:justify-between">
            <div>
              ...
              <p className="mt-2 text-sm text-slate-500">
                Fresh picks from the live shop API, ready to order.
              </p>
            </div>
          ...
          <div className="grid gap-8 lg:grid-cols-[0.8fr_1.2fr] lg:items-center">
            <div>
              ...
              <h2 className="mt-3 text-3xl font-bold text-white">
                A smarter way to buy home project essentials
              </h2>
              <p className="mt-3 text-sm leading-7 text-slate-400">
                From tools to finishing materials, our store helps customers
                find dependable products with clear pricing and fast support.
              </p>
            </div>
              ...
              {[
                {
                  ...
                  description:
                    "Products are pulled directly from the store API.",
                },
                {
                  title: "Premium selection",
                  description:
                    "Curated categories for repairs, finishes, and maintenance.",
                },
                {
                  title: "Easy ordering",
                  description:
                    "Simple product cards designed for quick product discovery.",
                },
                {
                  title: "Customer-first support",
                  description: "Helpful guidance before and after purchase.",
                },
              ].map((item) => (
                <div
                  key={item.title}
            ...
            <Accordion className="space-y-3">
              {faqs.map((faq, i) => (
                <AccordionItem
                  key={i}
                  value={`faq-${i}`}
                  className="rounded-xl border border-gray-200 bg-white px-6 data-[state=open]:border-lime-200 data-[state=open]:shadow-sm"
                >
          ...
          <MotionWrapper>
            <div className="mx-auto max-w-2xl text-center">
              ...
              <p className="mt-4 text-gray-400">
                Subscribe for stock updates, new products, and special offers.
              </p>
              <div className="mt-8">
                <NewsletterForm />
              </div>
      ...
      <Card className="group h-full overflow-hidden border border-slate-200 transition-all hover:-translate-y-1 hover:border-lime-200 hover:shadow-xl">
        <div className="relative aspect-square overflow-hidden bg-gradient-to-br from-slate-50 to-slate-100">
          ...
          <div className="absolute right-3 top-3 flex flex-col gap-2 opacity-0 transition-opacity group-hover:opacity-100">
            <button
              className="flex h-8 w-8 items-center justify-center rounded-full bg-white shadow-sm hover:bg-lime-50"
        ...
        <div className="p-5">
          <p className="text-xs font-semibold uppercase tracking-[0.25em] text-lime-600">
            {product.category}
          </p>
          ...
          <div className="mt-5 flex gap-2">
            <Button
              size="sm"
              className="flex-1 bg-lime-500 text-xs font-semibold text-white hover:bg-lime-600"
            >
              Add to cart
            </Button>
          </div>
