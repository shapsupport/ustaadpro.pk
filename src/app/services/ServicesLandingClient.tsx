    ...
    <section className="mx-auto max-w-7xl px-4 py-10 sm:px-6 lg:px-8">
      {initialSearch.trim() ? <>
        ...
        <div className="mt-6 grid grid-cols-2 gap-3 sm:gap-5 lg:grid-cols-4">{categories.map((category) => {
          ...
          return <Link key={category.id} href={`/services/${encodeURIComponent(category.id)}`} prefetch={false} className="group overflow-hidden rounded-2xl border border-slate-200 bg-white shadow-sm transition hover:border-emerald-300 hover:shadow-lg sm:rounded-3xl">
